...
Folder_var = tk.StringVar(value = "D:/git/MURBS_2026/ID43_MR_SG_20112020")
...
###                                 Tab1 --> Batch
# PROJECTS_ROOT Path
ttk.Label(tab1, text="PROJECT ROOT").grid(row=0, column=0, padx=5, pady=5)
ttk.Entry(tab1, textvariable=PROJECTS_ROOT_var, width=60).grid(row=0, column=1, padx=5)
ttk.Button(tab1, text="Browse", command=browse_PROJECTS_ROOT).grid(row=0, column=2)

# GIT_ROOT Path
ttk.Label(tab1,text="GIT_ROOT").grid(row=1, column=0, padx=5, pady=5)
ttk.Entry(tab1, textvariable=GIT_ROOT_var, width=60).grid(row=1, column=1)
ttk.Button(tab1, text="Browse", command=browse_GIT_ROOT).grid(row=1, column=2)

# TEMPLATES Path
ttk.Label(tab1, text="Html templates path").grid(row=2, column=0, padx=5, pady=5)
ttk.Entry(tab1, textvariable=TEMPLATES_var, width=60).grid(row=2, column=1, padx=5)
ttk.Button(tab1, text="Browse", command=browse_TEMPLATES).grid(row=2, column=2)

# IDD Path
ttk.Label(tab1, text="IDD path").grid(row=3, column=0, padx=5, pady=5)
ttk.Entry(tab1, textvariable=IDD_path_var, width=60).grid(row=3, column=1, padx=5)
ttk.Button(tab1, text="Browse", command=browse_IDD_path).grid(row=3, column=2)

## ---------- Radio Buttons
# the team_id --------------
ttk.Label(tab1, text="Archetype").grid(row=4, column=0)
ttk.Radiobutton(tab1,text="Single Family House",variable=teamid_var,value="poly").grid(row=4, column=1, sticky="w")
ttk.Radiobutton(tab1,text="Concordia",variable=teamid_var,value="Concordia").grid(row=5, column=1, sticky="w")


# Run Button
ttk.Button(tab1,text="Run All",command=run_process).grid(row=6, column=1, pady=20)


#-----------------------------
# the per_household_toggle --------------
ttk.Label(tab1, text="Simulation Aggregation").grid(row=4, column=1,padx=(100,0))
ttk.Radiobutton(tab1,text="Per Household",variable=per_household_var,value=1).grid(row=4, column=1,padx=(350,0), sticky="w")
ttk.Radiobutton(tab1,text="Whole Building",variable=per_household_var,value=0).grid(row=5, column=1,padx=(350,0), sticky="w")
#-----------------------------

####                                 Tab2 --> Single Building
# Folder Path
ttk.Label(tab2, text="Building Path").grid(row=0, column=0, padx=5, pady=5)
ttk.Entry(tab2, textvariable=Folder_var, width=60).grid(row=0, column=1, padx=5)
ttk.Button(tab2, text="Browse", command=browse_Folder).grid(row=0, column=2)
...
# Run Button
ttk.Button(tab2,text="Run Single Building",command=run_single_building).grid(row=6, column=1, pady=20)
