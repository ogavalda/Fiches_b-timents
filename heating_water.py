...
def generate_electricity_breakdown(path ,
                                   selected_day="08-15"): #will take the name of the model folder
    ...
    osm_path = f"{model_path}.osm"
    sql_path = (model_path / model_path /"run"/ "eplusout.sql")
    ...
    df_day = df[
        df.index.str.startswith(selected_day)
    ]
    ...
    csv_output = (
        output_folder /
        "hourly_electricity_breakdown.csv"
    )
    ...
    fig_output = (
        output_folder /
        "hourly_electricity_breakdown.png"
    )
...
generate_electricity_breakdown(
    r"ID39_MR_ND_1945",
    selected_day="08-15"
)